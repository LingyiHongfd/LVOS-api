...
from inference.data.test_datasets import LongTestDataset, DAVISTestDataset, YouTubeVOSTestDataset, LVOSTestDataset
...
parser.add_argument('--dataset', help='D16/D17/Y18/Y19/LV1/LV3/G/LVOS', default='D17')
...
is_lvos = args.dataset.startswith('LVOS')
...
if is_youtube:
    ...
    if args.dataset == 'LV1':
        meta_dataset = LongTestDataset(args.lv_path)
    ...
    meta_dataset = LVOSTestDataset(path.join(args.lvos_path), args.split)
...
for vid_reader in progressbar(meta_loader, max_value=len(meta_dataset), redirect_stdout=True):
    ...
    loader = DataLoader(vid_reader, batch_size=1, shuffle=False, num_workers=6)
    ...
    for ti, data in enumerate(loader):
        with torch.cuda.amp.autocast(enabled=not args.benchmark):
            ...
            out_mask = torch.argmax(prob, dim=0).indices
