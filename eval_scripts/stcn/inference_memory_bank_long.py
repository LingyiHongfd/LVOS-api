    ...
    def add_memory(self, key, value, is_temp=False):
        ...
        if self.mem_k is None:
            ...
            if is_temp:
                ...
                mem_size = self.mem_k.size(-1)
                mem_len = int(mem_size / self.size2d)
                if mem_len > 6:
                    self.mem_k = torch.cat([self.mem_k[:, :, :self.size2d], self.mem_k[:, :, self.size2d * 2:], key], 2)
                    self.mem_v = torch.cat([self.mem_v[:, :, :self.size2d], self.mem_v[:, :, self.size2d * 2:], value],
                                           2)
                else:
