...
# LVOS
from dataloaders.eval_datasets import (
    YOUTUBEVOS_Test,
    YOUTUBEVOS_DenseTest,
    DAVIS_Test,
    EVAL_TEST,
    LVOS_Test,
)
...
class Evaluator(object):
    ...
    def prepare_dataset(self):
        ...
        if len(cfg.TEST_MULTISCALE) > 1:
            eval_name += "_ms_" + str(cfg.TEST_MULTISCALE).replace(".", "dot").replace(
                "[", ""
            ).replace("]", "").replace(", ", "_")
        ...
        if "youtubevos" in cfg.TEST_DATASET:
            ...
            resolution = (
                "Full-Resolution" if cfg.TEST_DATASET_FULL_RESOLUTION else "480p"
            )
            self.result_root = os.path.join(
                cfg.DIR_EVALUATION,
                cfg.TEST_DATASET,
        ...
        if not os.path.exists(self.result_root):
            try:
                ...
                self.print_log(inst)
                self.print_log("Failed to mask dir: {}.".format(self.result_root))
        self.print_log("Done!")
    ...
    def evaluating(self):
        ...
        with torch.no_grad():
            for seq_idx, seq_dataset in enumerate(self.dataset):
                ...
                for frame_idx, samples in enumerate(seq_dataloader):
                    ...
                    for aug_idx in range(aug_num):
                        ...
                        if "current_label" in sample.keys():
                            current_label = (
                                ...
                                .cuda(self.gpu, non_blocking=True)
                                .float()
                            )
                        else:
                            current_label = None
                    ...
                    if frame_idx > 0:
                        ...
                        if cfg.TEST_FRAME_LOG:
                            ...
                            one_frametime = (
                                seq_timers[-1][0].elapsed_time(seq_timers[-1][1]) / 1e3
                            )
                            obj_num = obj_nums[0]
                            print(
                                "GPU {} - Frame: {} - Obj Num: {}, Time: {}ms".format(
                                    self.gpu,
                                    imgname[0].split(".")[0],
                                    obj_num,
                                    int(one_frametime * 1e3),
                                )
                            )
                        # Save result
                        ...
                        if (
                            "all_frames" in cfg.TEST_DATASET_SPLIT
                            and imgname in images_sparse
                        ):
                ...
                max_mem = torch.cuda.max_memory_allocated(device=self.gpu) / (
                    1024.0**3
                )
                print(
                    "GPU {} - Seq {} - FPS: {:.2f}. All-Frame FPS: {:.2f}, All-Seq FPS: {:.2f}, Max Mem: {:.2f}G".format(
                        self.gpu,
                        seq_name,
        ...
        if self.seq_queue is not None:
            ...
            if self.rank == 0:
                ...
                print(
                    "GPU {} - All-Frame FPS: {:.2f}, All-Seq FPS: {:.2f}, Max Mem: {:.2f}G".format(
                        list(range(self.gpu_num)),
                        1.0 / all_reduced_total_avg_time_per_frame,
                        1.0 / all_reduced_avg_sfps,
                        max_mem,
                    )
                )
        else:
            print(
                "GPU {} - All-Frame FPS: {:.2f}, All-Seq FPS: {:.2f}, Max Mem: {:.2f}G".format(
                    self.gpu, 1.0 / total_avg_time_per_frame, 1.0 / avg_sfps, max_mem
                )
            )
        ...
        if self.rank == 0:
            ...
            total_eval_time = str(
                datetime.timedelta(seconds=int(end_eval_time - start_eval_time))
            )
