...
class YOUTUBEVOS_DenseTest(object):
    ...
    def __getitem__(self, idx):
        ...
        images_sparse = []
        for obj_n in obj_names:
            images_sparse += map(lambda x: x + ".jpg", list(data[obj_n]["frames"]))
        images_sparse = np.sort(np.unique(images_sparse))

        images = np.sort(list(os.listdir(os.path.join(self.image_root, seq_name))))
        ...
        labels = np.sort(list(os.listdir(os.path.join(self.label_root, seq_name))))
...
class _EVAL_TEST(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if idx == 0:
            current_label = (current_frame_obj_num * np.ones((height, width))).astype(
                np.uint8
            )
            sample = {"current_img": current_img, "current_label": current_label}
        else:
...
class LVOS_Test(object):
    def __init__(
        ...
        if split == "val":
            self.label_root = os.path.join(root, split, "Annotations_convert")
        else:
