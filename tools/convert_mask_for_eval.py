...
#################################################
# convert LVOS val mask into YouTube-VOS format #
#################################################
...
def get_argparse():
    parser = argparse.ArgumentParser(description="Convert LVOS Eval")
    parser.add_argument(
        "--lvos_path", type=str, default=None, help="path to LVOS eval set"
    )
...
def main():
    ...
    lvos_mask_path = os.path.join(lvos_path, "Annotations")
    lvos_save_mask_path = os.path.join(lvos_path, "Annotations_convert")
    if os.path.exists(lvos_save_mask_path):
        shutil.rmtree(lvos_save_mask_path)
    os.makedirs(lvos_save_mask_path, exist_ok=True)
    videos = os.listdir(lvos_mask_path)

    for video in tqdm(videos):
        video_path = os.path.join(lvos_mask_path, video)
        video_save_path = os.path.join(lvos_save_mask_path, video)
        os.makedirs(video_save_path, exist_ok=True)

        masks = os.listdir(video_path)
        masks.sort()
        objs = []
        
        for mask in masks:
            mask_name = mask
            mask = Image.open(os.path.join(video_path, mask_name)).convert("P")
            mask = np.array(mask)
            _objs = np.unique(mask)
            ext_objs = []
            ...
            if len(ext_objs) > 0:
                ...
                for _o in ext_objs:
                    zero_mask[mask == _o] = _o
                zero_mask = Image.fromarray(zero_mask).convert("P")
                zero_mask.putpalette(_palette)
                zero_mask.save(os.path.join(video_save_path, mask_name))
